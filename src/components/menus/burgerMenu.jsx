...
export default function BurgerMenu ({menu, setMenu , setHamburger}) {
    ...
    return (
        <>
            <div className="hidden fixed w-full h-full left-0 top-0 z-[90] bg-black/60 cursor-pointer md:block"
                    style={menu ? {opacity:1, transition:'opacity 1s'} : {opacity:0, zIndex:-10}} onClick={() => {setMenu(false);unlock();setHamburger(false)}}></div>
                <menu className="hidden fixed pl-10 w-[60%] pr-10 h-full top-0 bg-white z-[100] py-16 flex-col items-cente text-secondary md:flex sm:w-full sm:pr-10 md:py-10"
