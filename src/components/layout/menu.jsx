...
export default function Menu ({menu, setMenu , setHamburger}) {
    ...
    return (
        <>
                <div className="absolute w-screen h-screen transition-all duration-500 z-30 top-0 bg-black/80 cursor-pointer"
                    style={menu ? {left:"0%"} : {left:"-100%"}} onClick={() => {setMenu(false);unlock();setHamburger(false)}}></div>
                <menu className="fixed pl-10 pr-10 h-full top-0 bg-white transition-all duration-1000 z-40 py-16 flex-col items-center sm:w-full sm:pr-10 md:py-10" style={menu ? {left:"0%"} : {left:"-100%"}}>
